...
%             end
        ...
        %% Calculate muscle length and velocity to set up muscle parameters
        function set_muscle_parameters(obj)
            % Store joint motion in joint objects
            ...
            %%% Store joint position in joint objects            
            ...
            pos_knee = sizer;
            pos_ankle = sizer;
%             % Initail position of the bodies
%             pos_femur_in = (obj.Cabs_bodies(:,:,1)*obj.pos_bodies(:,2))';
%             pos_tibia_in = pos_femur_in + (obj.Cabs_bodies(:,:,2)*obj.pos_bodies(:,3))';
%             pos_foot_in = pos_tibia_in + (obj.Cabs_bodies(:,:,3)*obj.pos_bodies(:,4))';
%             %Initial positoion of the joints
%             pos_hip_in = pos_femur_in + (obj.Cabs_bodies(:,:,2)*obj.pos_joints(:,2))';
%             pos_knee_in = pos_tibia_in + (obj.Cabs_bodies(:,:,3)*obj.pos_joints(:,3))';
%             pos_ankle_in = pos_foot_in + (obj.Cabs_bodies(:,:,4)*obj.pos_joints(:,4))';
            motion_calculation(obj)
            keyboard
            ...
            for i = 1:length(obj.dt_motion)
                ...
                pos_femur(i,:) = obj.Cabs_bodies(:,:,1)*a*obj.pos_bodies(:,2);
                pos_hip(i,:) = pos_femur(i,:) + (obj.Cabs_bodies(:,:,2)*a*obj.pos_joints(:,2))';
                pos_tibia(i,:) = pos_femur(i,:) + (obj.Cabs_bodies(:,:,2)*obj.pos_bodies(:,3))';
                pos_knee(i,:) = pos_tibia(i,:)+ (obj.Cabs_bodies(:,:,3)*obj.pos_joints(:,3))';
                pos_foot(i,:) = pos_tibia(i,:) + (obj.Cabs_bodies(:,:,3)*obj.pos_bodies(:,4))';
                pos_ankle(i,:) = pos_foot(i,:) +(obj.Cabs_bodies(:,:,4)*obj.pos_joints(:,4))';
